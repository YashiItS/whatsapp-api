...
// Send a manual hello_world test message
app.post('/api/test-whatsapp', async (req, res) => {
  const recipientPhoneNumber = req.body?.to;

  if (typeof recipientPhoneNumber !== 'string' || !recipientPhoneNumber.trim()) {
    res.status(400).json({error: 'The request body must include a non-empty "to" field.'});
    return;
  }

  try {
    const response = await GraphApi.sendHelloWorldMessage(recipientPhoneNumber.trim());
    res.status(200).json(response);
  } catch (error) {
    console.error('Test WhatsApp message failed:', error.message);
    res.status(502).json({error: 'WhatsApp message could not be sent.'});
  }
});
